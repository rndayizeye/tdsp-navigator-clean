...
def _(
    ...
    **5. Vision Zero fatality rate trend**
    ...
    | Priority | Location Type | Agency | Intervention | Timeline |
    |----------|--------------|--------|--------------|----------|
    | **1 — Highest leverage** | Isolated surface street hotspots ({len(iso)} sites) | NYC DOT / NYPD | Leading pedestrian intervals, signal retiming, crosswalk hardening, road diet | 1–3 years |
    | **2 — Corridor safety** | Belt Parkway, Major Deegan, BQE, FDR Drive | NYS DOT / MTA | Automated speed enforcement, managed speed zones, rumble strips | 3–5 years |
    | **3 — Pedestrian protection** | Broadway, Atlantic Ave, Flatbush Ave | NYC DOT | Road diet, pedestrian refuge islands, exclusive pedestrian phases | 1–2 years |
    | **4 — Cyclist safety** | Northern Blvd, Queens Blvd, Bedford Ave | NYC DOT | Protected bike lanes, intersection hardening, bike signal priority | 1–2 years |
    | **5 — Data quality** | All corridors | NYPD | Improve `on_street_name` completion rate in crash reports (currently ~75%) | Immediate |

    ### Implementation Strategy

    **Phase 1 (Year 1): Quick Wins**
    - Deploy leading pedestrian intervals at top 50 isolated hotspots
    - Install automated speed enforcement on Belt Parkway and Major Deegan
    - Implement signal retiming at high-fatality intersections

    **Phase 2 (Years 2–3): Infrastructure Hardening**
    - Construct pedestrian refuge islands on wide arterials
    - Build protected bike lanes on VRU-priority corridors
    - Execute road diets on oversized surface streets

    **Phase 3 (Years 3–5): Systemic Change**
    - Corridor-wide redesign of highway systems (state partnership required)
    - Area-wide speed limit reductions in high-crash neighborhoods
    - Mode shift investments to reduce vehicle volume on high-crash corridors

    ### Funding & Authority Considerations

    - **City-controlled interventions** (isolated hotspots, surface streets): Can be executed with
      existing NYC DOT capital and expense budgets. Primary barrier is staff capacity, not funding.

    - **State-controlled interventions** (highway corridors): Require coordination with NYS DOT and
      MTA Bridges & Tunnels. May require state legislative action for speed limit changes.

    - **Enforcement interventions**: NYPD Traffic Division capacity is the limiting factor.
      Automated enforcement (cameras) can partially substitute for officer presence.
